...
export const ServiceDetailPage: React.FC = () => {
  ...
  // Find service by id or fallback to first
      ...
      <section className="relative min-h-[48vh] sm:min-h-[52vh] flex items-center justify-center pt-28 pb-16 overflow-hidden bg-[#0A2540]">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center mt-6">
          <motion.h1 
            ...
            className="text-2xl sm:text-4xl lg:text-5xl font-normal text-white tracking-[-0.035em] leading-[1.16] mb-4"
          >
            {service.title}
          </motion.h1>
          ...
          >
            ...
            <span>/</span>
            <span className="text-emerald-400 font-medium truncate max-w-[200px]">{service.title}</span>
          </motion.div>
        </div>
      </section>
                  ...
                  <div className="space-y-4">
                    {service.challenges.map((item, idx) => (
                      ...
                      >
                        ...
                        <div className="flex items-start gap-2.5 text-xs sm:text-sm font-light text-zinc-700 pl-6 border-l-2 border-emerald-500">
                          <span>Solución ECO HOGAR: {item.solution}</span>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              <div className="bg-[#0A2540] text-white rounded-3xl p-6 sm:p-8 space-y-5 shadow-xl border border-emerald-500/30">
                ...
                <p className="text-xs font-light text-zinc-300 leading-relaxed">
                  Comunícate directamente con nuestro equipo de ventas para resolver cualquier duda o agendar tu visita gratuita.
                </p>
                ...
                <div className="space-y-2.5">
                  {otherServices.map((os) => (
                    <Link
                      key={os.id}
                      to={`/servicios/${os.id}`}
                      className="block p-3 rounded-xl bg-white border border-zinc-100 hover:border-emerald-500/40 transition-colors text-xs font-medium text-zinc-800 hover:text-emerald-700"
                    >
                      {os.title}
                    </Link>
                  ))}
