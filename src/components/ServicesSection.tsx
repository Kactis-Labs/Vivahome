      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-xs sm:text-sm font-light text-zinc-600 max-w-md leading-relaxed">
            Estructuramos cada operación para maximizar el valor de tu inversión y proteger tu patrimonio en cada etapa.
          </p>
        </motion.div>
          ...
          {featuredServices.map((service, index) => (
            ...
            >
              ...
              <Link to={`/servicios/${service.id}`} className="relative aspect-[16/10] overflow-hidden block bg-zinc-100">
                ...
                <div className="absolute bottom-4 right-4 w-10 h-10 rounded-full bg-lime-500 text-black flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:bg-lime-400 transition-all">
                  <ArrowUpRight className="w-5 h-5 stroke-[2.5]" />
                </div>

                <div className="absolute top-4 left-4 bg-[#0C130E]/85 backdrop-blur-md text-lime-400 text-[10px] font-semibold px-3 py-1 rounded-md uppercase tracking-wider border border-lime-400/30">
                  {service.tag}
                </div>
              </Link>
              ...
              <div className="p-6 sm:p-7 flex-1 flex flex-col justify-between">
                <div>
                  <Link to={`/servicios/${service.id}`}>
                    <h3 className="text-lg sm:text-xl font-medium tracking-tight text-zinc-900 group-hover:text-lime-700 transition-colors mb-2.5">
                      {service.title}
                    </h3>
                  </Link>
                ...
                <div className="pt-4 border-t border-zinc-100 flex items-center justify-between">
                  <Link 
                    ...
                    className="text-xs font-medium text-lime-700 group-hover:text-lime-800 transition-colors flex items-center gap-1"
                  >
