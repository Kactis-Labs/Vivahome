      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <span className="text-xs font-semibold text-emerald-700 uppercase tracking-wider block mb-2 sm:mb-3">
            ASESORES DE CAMPO & VENTAS
          </span>
          <h2 className="text-2xl sm:text-4xl lg:text-5xl font-medium tracking-tight text-black mb-2 sm:mb-4">
            Conoce al Equipo de ECO HOGAR Inmobiliaria
          </h2>
          <p className="text-xs sm:text-base font-light text-zinc-600 leading-relaxed">
            Profesionales comprometidos en asesorarte y acompañarte en cada visita a nuestros proyectos en La Libertad.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-10">
          {agents.map((agent, index) => (
            <motion.div
              ...
              className="group bg-[#F8FAFC] border border-zinc-200 rounded-3xl p-6 flex flex-col justify-between hover:border-emerald-500/40 hover:shadow-xl transition-all duration-300 hover:-translate-y-1.5"
            >
              <div>
                {/* Agent Image */}
                <Link to={`/equipo/${agent.id}`} className="block relative aspect-square rounded-2xl overflow-hidden mb-5 bg-zinc-100">
                  ...
                  <div className="absolute bottom-3 right-3 w-9 h-9 rounded-full bg-emerald-500 text-white flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
                ...
                <Link to={`/equipo/${agent.id}`}>
                  <h3 className="text-lg sm:text-xl font-medium tracking-tight text-zinc-900 group-hover:text-emerald-700 transition-colors">
                    {agent.name}
                  </h3>
                </Link>
