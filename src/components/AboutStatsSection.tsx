      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="text-2xl sm:text-4xl lg:text-6xl font-normal text-slate-900 leading-[1.18] tracking-[-0.035em]">
            Creamos comunidades residenciales de campo y playa en Puerto Malabrigo con diseño de Estilo Americano y financiamiento directo sin intereses.
          </h2>

          <div className="mt-6 sm:mt-8 flex items-center gap-4">
            <a 
              href="/nosotros"
              className="inline-flex items-center gap-2 bg-[#E5A020] hover:bg-[#D97706] text-white font-medium text-xs sm:text-sm px-5 sm:px-7 py-2.5 sm:py-3 rounded-full transition-all shadow-md hover:scale-105"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 items-center pt-6 sm:pt-8 border-t border-slate-200">
            ...
            <div className="relative rounded-2xl sm:rounded-3xl overflow-hidden shadow-xl group">
              ...
              <div className="absolute bottom-3 left-3 right-3 sm:bottom-6 sm:left-6 sm:right-6 p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-white/95 backdrop-blur-md border border-white/50 flex items-center justify-between shadow-lg">
                <div className="flex items-center gap-2.5 sm:gap-3">
                  <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-[#E5A020] text-white flex items-center justify-center font-bold shrink-0">
                    <ShieldCheck className="w-4 h-4 sm:w-5 sm:h-5" />
                  </div>
                  <div className="overflow-hidden">
                    <h4 className="text-xs sm:text-sm font-medium text-slate-900 truncate">Villa Club Malabrigo</h4>
                    <p className="text-[10px] sm:text-xs font-light text-slate-600 truncate">El único proyecto con Estilo Americano</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-5 space-y-4 sm:space-y-6 pl-0 lg:pl-6">
            {stats.map((stat, idx) => (
              ...
              >
                <div className="text-3xl sm:text-5xl font-medium text-[#1D4263] tracking-tight mb-1 sm:mb-2 font-mono font-bold">
                  {stat.value}
                </div>
