        ...
        >
          <div>
            <div className="inline-flex items-center gap-2 px-3 sm:px-3.5 py-1 rounded-full bg-white border border-slate-200 text-slate-800 text-[10px] sm:text-xs font-medium uppercase tracking-wider mb-3 sm:mb-4 shadow-sm">
              <BookOpen className="w-3.5 h-3.5 text-[#E5A020]" />
              <span>INSIGHTS DEL MERCADO</span>
            </div>
            <h2 className="text-2xl sm:text-4xl lg:text-5xl font-medium tracking-tight text-slate-900">
              Novedades & Avance de Obras
            </h2>
          </div>

          <a 
            href="#deals"
            className="inline-flex items-center gap-2 bg-[#1D4263] hover:bg-[#132B45] text-white font-medium text-xs sm:text-sm px-5 sm:px-6 py-2.5 rounded-full transition-all shadow-sm hover:shadow-md self-start md:self-auto"
          >
              ...
              <div className="p-5 sm:p-8 flex-1 flex flex-col justify-between">
                <div>
                  <div className="flex items-center gap-1.5 text-xs font-light text-slate-500 mb-2 sm:mb-3">
                    <Calendar className="w-3.5 h-3.5 text-[#E5A020]" />
                    <span>{post.date}</span>
                  </div>

                  <Link to={`/blog/${post.id}`}>
                    <h3 className="text-base sm:text-lg font-medium text-slate-900 mb-2 sm:mb-3 group-hover:text-amber-600 transition-colors leading-snug">
                      {post.title}
                    </h3>
                  </Link>
                ...
                <div className="pt-4 sm:pt-6 mt-4 sm:mt-6 border-t border-slate-100 flex items-center justify-between">
                  <Link 
                    to={`/blog/${post.id}`}
                    className="text-xs font-semibold text-slate-900 group-hover:text-amber-600 transition-colors flex items-center gap-1.5"
                  >
                    <span>Leer Artículo Completo</span>
                    <ArrowUpRight className="w-3.5 h-3.5" />
                  </Link>
                  <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-slate-100 group-hover:bg-[#E5A020] group-hover:text-white flex items-center justify-center transition-colors">
