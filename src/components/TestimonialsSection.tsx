...
export const TestimonialsSection: React.FC = () => {
  ...
  return (
    <section className="py-16 sm:py-24 bg-[#FAF8F5] text-slate-900 border-t border-[#E8E4DC] overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="text-2xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-[#1C1917] mb-2 sm:mb-4">
            +420 Familias que Viven la Experiencia R&R
          </h2>
          <p className="text-slate-600 text-xs sm:text-base font-normal">
            Historias reales de propietarios e inversionistas que eligieron la calidad y respaldo de nuestros proyectos.
          </p>
        </motion.div>
          ...
          {testimonials.map((test, idx) => {
            ...
            return (
              <motion.div 
                ...
                whileHover={{ y: -4 }}
                className={`bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-8 border border-[#E8E4DC] hover:border-[#C8A97E] transition-all flex flex-col justify-between shadow-sm hover:shadow-md ${
                  isHiddenMobile ? 'hidden sm:flex' : 'flex'
                }`}
              >
                <div>
                  {/* 5-Star Rating in Gold */}
                  <div className="flex items-center gap-1 mb-3 text-[#C8A97E]">
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-[#E8E4DC]">
                  <img 
                    src={test.avatar} 
                    alt={test.name} 
                    className="w-10 h-10 sm:w-11 sm:h-11 rounded-full object-cover ring-2 ring-[#C8A97E]/40 shrink-0"
                  />
                  <div className="overflow-hidden">
                    <h4 className="text-xs sm:text-sm font-bold text-[#1C1917] truncate">{test.name}</h4>
