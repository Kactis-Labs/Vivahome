      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="text-2xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-[#111827] mb-2 sm:mb-4">
            +1,500 Clientes que Confían en JML
          </h2>
          <p className="text-slate-600 text-xs sm:text-base font-normal">
            Historias reales de compradores y propietarios que lograron su meta inmobiliaria en Trujillo.
          </p>
        </motion.div>
          ...
          {testimonials.map((test, idx) => {
            ...
            return (
              <motion.div 
                ...
                whileHover={{ y: -4 }}
                className={`bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-8 border border-slate-200 hover:border-[#DC2626] transition-all flex flex-col justify-between shadow-sm hover:shadow-md ${
                  isHiddenMobile ? 'hidden sm:flex' : 'flex'
                }`}
              >
                <div>
                  {/* 5-Star Rating in Red/Amber */}
                  <div className="flex items-center gap-1 mb-3 text-[#DC2626]">
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-slate-100">
                  <img 
                    src={test.avatar} 
                    alt={test.name} 
                    className="w-10 h-10 sm:w-11 sm:h-11 rounded-full object-cover ring-2 ring-[#DC2626]/40 shrink-0"
                  />
                  <div className="overflow-hidden">
                    <h4 className="text-xs sm:text-sm font-bold text-[#111827] truncate">{test.name}</h4>
