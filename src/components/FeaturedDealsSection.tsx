        ...
        >
          <div>
            <span className="text-xs font-bold text-[#DC2626] uppercase tracking-wider block mb-2">
              PORTAFOLIO EXCLUSIVO EN TRUJILLO
            </span>
          ...
          <p className="text-slate-600 max-w-md text-xs sm:text-sm font-normal leading-relaxed">
            Departamentos, casas y residencias de primer nivel en El Golf, California, San Andrés y principales zonas de Trujillo.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {featuredProperties.map((prop, idx) => (
            <motion.div 
              ...
              whileHover={{ y: -6 }}
              className="group bg-white rounded-2xl sm:rounded-3xl overflow-hidden border border-slate-200 hover:border-[#DC2626] transition-all duration-300 flex flex-col justify-between shadow-sm hover:shadow-xl hover:shadow-[#DC2626]/10"
            >
              {/* Image Container */}
              <Link to={`/propiedades/${prop.id}`} className="block relative aspect-[16/11] overflow-hidden bg-slate-100">
                ...
                {/* Badge Overlay */}
                {prop.badge && (
                  <div className="absolute top-3 left-3 sm:top-4 sm:left-4 bg-[#111827]/90 text-[#EF4444] border border-[#EF4444]/30 text-[10px] sm:text-[11px] font-bold px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full uppercase tracking-wider backdrop-blur-md">
                    {prop.badge}
                  </div>
                )}
                ...
                <div className="absolute bottom-3 left-3 right-3 sm:bottom-4 sm:left-4 sm:right-4 flex items-baseline justify-between">
                  ...
                  <span className="text-[10px] sm:text-xs font-bold text-white bg-[#DC2626] px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full shadow-sm">
                    {prop.category}
                  </span>
                </div>
              </Link>
              ...
              <div className="p-5 sm:p-6 flex-1 flex flex-col justify-between">
                <div>
                  <Link to={`/propiedades/${prop.id}`}>
                    <h3 className="text-base sm:text-lg font-bold text-[#111827] group-hover:text-[#DC2626] transition-colors mb-1.5 sm:mb-2 line-clamp-1">
                      {prop.title}
                    </h3>
                  </Link>
                ...
                <div className="pt-3 sm:pt-4 border-t border-slate-100 flex items-center justify-between text-[11px] sm:text-xs text-slate-600 font-normal">
                  {prop.beds > 0 && (
                    <div className="flex items-center gap-1">
                      <Bed className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#DC2626]" />
                      <span>{prop.beds} Dorm.</span>
                    </div>
                  )}
                  {prop.baths > 0 && (
                    <div className="flex items-center gap-1">
                      <Bath className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#DC2626]" />
                      <span>{prop.baths} Baños</span>
                    </div>
                  )}
                  <div className="flex items-center gap-1">
                    <Maximize2 className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#DC2626]" />
                ...
                <div className="mt-4 sm:mt-6 pt-3 sm:pt-4 flex items-center gap-2.5 sm:gap-3">
                  <a 
                    href={`https://wa.me/51987654321?text=Hola,%20deseo%20mas%20informacion%20del%20inmueble%20en%20Trujillo:%20${encodeURIComponent(prop.title)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1 bg-[#111827] hover:bg-[#DC2626] text-white rounded-full px-3 sm:px-4 py-2.5 text-[11px] sm:text-xs font-bold transition-all flex items-center justify-center gap-1.5 sm:gap-2 truncate shadow-sm"
                  >
                  ...
                  <Link 
                    ...
                    className="w-9 h-9 rounded-full bg-slate-100 hover:bg-[#DC2626] hover:text-white text-slate-700 flex items-center justify-center transition-all shrink-0 shadow-sm"
